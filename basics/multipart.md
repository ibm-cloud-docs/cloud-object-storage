...
This is the same wether you use the example file or the one generated by running: s3cmd --configure.
...
Basic usage information is available on the [s3tools](https://s3tools.org/usage){:new_window} site.

### Multi-Part Upload using S3cmd

A `put` command will automatically execute a multi-part upload when attempting to upload a large file.
