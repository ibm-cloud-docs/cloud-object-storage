...
Presigned URLs create a temporary link that can be used to share an object publicly, or to provide a temporary target for sending a PUT request without needing to provide authentication information.
...
endpoint = 'https://' + host
object_key = 'example-object'
object_contents = 'this is a temporary object to be overwritten'
...
# generate the signature
raw_signature = hmac.new(secret_key, sts.encode('utf-8'), hashlib.sha1).digest()

signature = quote(base64.b64encode(raw_signature), safe='')

# create and send the request
# the 'requests' package autmatically adds the required 'host' header
presign = 'AWSAccessKeyId=' + access_key + '&Signature=' + signature + '&Expires=' + str(expires)
request_url = endpoint + '/' + bucket + '/' + object_key + '?' + presign
print 'request_url: %s' % request_url
...
print 'bucket: %s' % bucket
print 'object_key: %s' % object_key
print 'object_contents: %s' % object_contents
print 'request_parameters: %s' % request_parameters

print 'Standardized request details ----------------------------'
print 'standardized_resource: %s' % standardized_resource
