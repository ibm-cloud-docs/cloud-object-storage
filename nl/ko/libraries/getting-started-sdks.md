...
이 빠른 시작 안내서에서는 다음 오퍼레이션을 시연하는 코드 예를 제공합니다. 
...
* [{{site.data.keyword.cloud}} 플랫폼 계정](https://cloud.ibm.com/login)
* [{{site.data.keyword.cos_full_notm}}의 인스턴스](/docs/services/cloud-object-storage/basics?topic=cloud-object-storage-provision)
* {{site.data.keyword.cos_short}}에 대한 Writer 액세스 권한이 있는 [IAM API 키](/docs/services/cloud-object-storage/iam?topic=cloud-object-storage-iam-overview)

## SDK 가져오기
{: #sdk-gs-install}

SDK를 다운로드하여 설치하는 데 대한 구체적인 지시사항은 [Python 사용에 있습니다. ](/docs/services/cloud-object-storage/libraries?topic=cloud-object-storage-python){:new_window}{: python}[Node.js 사용에 있습니다. ](/docs/services/cloud-object-storage/libraries?topic=cloud-object-storage-node){:new_window}{: javascript}[Java 사용에 있습니다. ](/docs/services/cloud-object-storage/libraries?topic=cloud-object-storage-java){:new_window}{: java}[Go 사용에 있습니다. ](/docs/services/cloud-object-storage/libraries?topic=cloud-object-storage-go){:new_window}{: go}

## 코드 예
{: #sdk-gs-example}

아래 코드 예는 {{site.data.keyword.cos_short}}에 대해 기본 오퍼레이션을 실행하는 방법을 소개하는 예를 제공합니다. 코드 예는 잠재적 충돌을 방지하기 위해 버킷/항목 이름에 UUID(Universally Unique Identifier)를 사용하므로, 작업을 간소화하기 위해 코드 예를 여러 번 실행할 수 있습니다. 

코드 예를 완료하려면 다음 값을 대체해야 합니다. 

|값|설명|예|
|---|---|---|
|`<endpoint>`|COS 인스턴스의 지역 엔드포인트|`s3.us-south.cloud-object-storage.appdomain.cloud`|
|`<api-key>`|`Writer` 이상의 권한이 있는 IAM API 키|`xxxd12V2QHXbjaM99G9tWyYDgF_0gYdlQ8aWALIQxXx4`|
|`<resource-instance-id>`|서비스 인스턴스의 고유 ID|`crn:v1:bluemix:public:cloud-object-storage:global:a/xx999cd94a0dda86fd8eff3191349999:9999b05b-x999-4917-xxxx-9d5b326a1111::`|
|`<storage-class>`|새 버킷의 스토리지 클래스|`us-south-standard`|
...
코드 샘플을 실행하려면 위 코드 블록을 복사한 후 다음 명령을 실행하십시오. 
```
