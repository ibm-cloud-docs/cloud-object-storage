...
事前署名 URL によって、一時リンクが作成されます。この一時リンクを使用して、オブジェクトをパブリックで共有したり、認証情報を提供せずに、[PUT 要求を送信するための一時的な宛先を指定したり](https://medium.com/codait/keeping-your-secrets-between-cloud-object-storage-and-your-browser-part-1-68f4b83bbd38)できます。

事前署名 URL を作成する最も簡単な方法は、[AWS CLI](/docs/services/cloud-object-storage/cli?topic=cloud-object-storage-aws-cli) を使用することです。
...
また、プログラムで組み立てることもできます。Python で書かれた基本的な `GET` 操作の例を以下に示します。エンドポイントについて詳しくは、[エンドポイントおよびストレージ・ロケーション](/docs/services/cloud-object-storage?topic=cloud-object-storage-endpoints#endpoints)を参照してください。

## オブジェクトをダウンロードするための事前署名 URL の作成
{: #presign-url-get}
...
## オブジェクトをアップロードするための事前署名 URL の作成
