...
このクイック・スタート・ガイドには、以下の操作を実演するサンプル・コードが用意されています。
...
* [{{site.data.keyword.cloud}} プラットフォーム・アカウント](https://cloud.ibm.com/login)
* [{{site.data.keyword.cos_full_notm}} のインスタンス](/docs/services/cloud-object-storage/basics?topic=cloud-object-storage-provision)
* {{site.data.keyword.cos_short}} へのライター権限を持つ [IAM API キー](/docs/services/cloud-object-storage/iam?topic=cloud-object-storage-iam-overview)

## SDK の取得
{: #sdk-gs-install}

SDK のダウンロードおよびインストールに関する具体的な説明は、[Python の使用](/docs/services/cloud-object-storage/libraries?topic=cloud-object-storage-python){:new_window}{: python}[Node.js の使用](/docs/services/cloud-object-storage/libraries?topic=cloud-object-storage-node){:new_window}{: javascript}[Java の使用](/docs/services/cloud-object-storage/libraries?topic=cloud-object-storage-java){:new_window}{: java}[Go の使用](/docs/services/cloud-object-storage/libraries?topic=cloud-object-storage-go){:new_window}{: go} にあります。

## サンプル・コード
{: #sdk-gs-example}

以下のサンプル・コードは、{{site.data.keyword.cos_short}} で基本的な操作を実行する場合の初歩的な例を示しています。便宜上、サンプル・コードでは、バケット/項目名に汎用固有 ID (UUID) を使用することで競合が発生する可能性を回避しているため、サンプル・コードは複数回実行できます。

サンプル・コードを完成させるには、以下の値を置き換える必要があります。

|値|説明|例|
|---|---|---|
|`<endpoint>`|COS インスタンスの地域エンドポイント|`s3.us-south.cloud-object-storage.appdomain.cloud`|
|`<api-key>`|最低でも`ライター`の許可を持つ IAM API キー|`xxxd12V2QHXbjaM99G9tWyYDgF_0gYdlQ8aWALIQxXx4`|
|`<resource-instance-id>`|サービス・インスタンスの固有 ID|`crn:v1:bluemix:public:cloud-object-storage:global:a/xx999cd94a0dda86fd8eff3191349999:9999b05b-x999-4917-xxxx-9d5b326a1111::`|
|`<storage-class>`|新規バケットのストレージ・クラス|`us-south-standard`|
...
コード・サンプルを実行するには、上記のコード・ブロックをコピーし、以下を実行します。
```
