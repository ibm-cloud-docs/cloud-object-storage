...
# Criar uma URL pré-assinada
{: #presign-url}

As URLs pré-assinadas criam um link temporário que pode ser usado para compartilhar um objeto publicamente ou para [fornecer um destino temporário para enviar uma solicitação de PUT](https://medium.com/codait/keeping-your-secrets-between-cloud-object-storage-and-your-browser-part-1-68f4b83bbd38) sem a necessidade de fornecer informações de autenticação.

A maneira mais fácil de criar URLs pré-assinadas está usando a [CLI do AWS](/docs/services/cloud-object-storage/cli?topic=cloud-object-storage-aws-cli):
...
Também é possível construí-las programaticamente. Aqui estão exemplos para as operações básicas `GET` gravadas em Python. Para obter mais informações sobre terminais, consulte [Terminais e locais de armazenamento](/docs/services/cloud-object-storage?topic=cloud-object-storage-endpoints#endpoints).

## Criar uma URL pré-assinada para fazer download de um objeto
{: #presign-url-get}
...
## Criar uma URL pré-assinada para fazer upload de um objeto
