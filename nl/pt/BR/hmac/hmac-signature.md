...
# Construindo uma assinatura HMAC
{: #hmac-signature}

Cada solicitação feita com relação ao IBM COS usando [credenciais HMAC](/docs/services/cloud-object-storage/hmac?topic=cloud-object-storage-hmac) em vez de uma [chave de API ou token de acesso](/docs/services/cloud-object-storage/iam?topic=cloud-object-storage-iam-overview) deve ser autenticada usando uma implementação do cabeçalho `authorization` do AWS Signature Versão 4. O uso de uma assinatura fornece verificação de identidade e integridade de dados em trânsito e, como cada assinatura é ligada ao registro de data e hora da solicitação, não é possível reutilizar cabeçalhos de autorização. O cabeçalho é composto de quatro componentes: uma declaração de algoritmo, informações de credenciais, cabeçalhos assinados e a assinatura calculada:
...
A data é fornecida no formato `YYYYMMDD` e a região deve corresponder ao local do depósito especificado, por exemplo, `us`. Os cabeçalhos `host` e `x-amz-date` são sempre necessários e, dependendo da solicitação, outros cabeçalhos também podem ser necessários (por exemplo, `x-amz-content-sha256` no caso de solicitações com cargas úteis). Devido à necessidade de recalcular a assinatura para cada solicitação individual, muitos desenvolvedores preferem usar uma ferramenta ou um SDK que produzirá o cabeçalho de autorização automaticamente.

## Criando um cabeçalho `authorization`
{: #hmac-auth-header}

Primeiro, precisamos criar uma solicitação em um formato padronizado.

1. Declare qual método de HTTP estamos usando (por exemplo, `PUT`)
2. Defina o recurso que estamos acessando em um modo padronizado. Esta é a parte do endereço em `http(s)://` e a sequência de consultas. Para solicitações no nível de conta (ou seja, listar depósitos), isso é simplesmente `/`.
3. Se houver quaisquer parâmetros de solicitação, eles deverão ser padronizados sendo codificados por percentual (por exemplo, os espaços deverão ser representados como `%20`) e ordenados alfabeticamente.
4. Os cabeçalhos precisam ser padronizados removendo espaço em branco, convertendo em minúsculas e incluindo uma nova linha em cada um deles, em seguida, eles devem ser classificados em ordem ASCII.
5. Depois de serem listados em um formato padrão, eles devem ser 'assinados'. Isso está considerando apenas os nomes de cabeçalho, não seus valores, e listando-os em ordem alfabética, separados por ponto e vírgula. `Host` e `x-amz-date` são necessários para todas as solicitações.
6. Se a solicitação tiver um corpo, como ao fazer upload de um objeto ou criar uma nova ACL, o corpo da solicitação deverá ser em hash usando o algoritmo SHA-256 e representado como caracteres minúsculos codificados em base-16.
7. Combinar o método de HTTP, o recurso padronizado, os parâmetros padronizados, os cabeçalhos padronizados, os cabeçalhos assinados e o corpo da solicitação em hash, cada um separado por uma nova linha para formar uma solicitação padronizada.

Em seguida, precisamos montar um 'string-to-sign', que será combinado com a chave de assinatura para formar a assinatura final. O string-to-sign usa o formato a seguir:
...
1. O horário deve ser UTC atual e formatado de acordo com a especificação ISO 8601 (por exemplo, `20161128T152924Z`).
2. A data está no formato `YYYYMMDD`.
3. A linha final é a solicitação padronizada criada anteriormente em hash usando o algoritmo SHA-256.

Agora, precisamos realmente calcular a assinatura.

1. Primeiro, a chave de assinatura precisa ser calculada por meio da chave de acesso secreta da conta, a data atual e a região e o tipo de API que estão sendo usados.
2. A sequência `AWS4` é pré-anexada à chave de acesso secreta e, em seguida, essa nova sequência é usada como a chave para efetuar hash da data.
3. Em seguida, o hash resultante é usado como a chave para efetuar hash da região.
4. O processo continua com o novo hash sendo usado como a chave para efetuar hash do tipo de API.
5. Finalmente, o hash mais recente é usado como a chave para efetuar hash da sequência `aws4_request` criando a chave de assinatura.
6. A chave de assinatura é, então, usada como a chave para efetuar hash do string-to-sign que gera a assinatura final.

Agora, a única etapa restante é realmente montar o cabeçalho `authorization` conforme mostrado:

```
AWS4-HMAC-SHA256 Credential={access-key}/{date}/{region}/s3/aws4_request,SignedHeaders=host;x-amz-date;{other-required-headers},Signature={signature}
```

## Gerando um cabeçalho `authorization`
