...
# Introduzione agli SDK
{: #sdk-gs}

In questa guida rapida, ti viene fornito un esempio di codice che illustra le seguenti operazioni:

* Crea un nuovo bucket
* Elenca i bucket disponibili
* Crea un nuovo file di testo
* Elenca i file disponibili
* Richiama il contenuto del file di testo
* Carica un file binario di grandi dimensioni
* Elimina un file
* Elimina un bucket
...
* Un account [{{site.data.keyword.cloud}} Platform](https://cloud.ibm.com/login)
...
* Una [chiave API IAM](/docs/services/cloud-object-storage/iam?topic=cloud-object-storage-iam-overview) con accesso Writer (Scrittore) al tuo {{site.data.keyword.cos_short}}

## Ottenimento dell'SDK
{: #sdk-gs-install}

Le specifiche istruzioni per scaricare e installare l'SDK sono disponibili in [Utilizzo di Python.](/docs/services/cloud-object-storage/libraries?topic=cloud-object-storage-python){:new_window}{: python}[Utilizzo di Node.js.](/docs/services/cloud-object-storage/libraries?topic=cloud-object-storage-node){:new_window}{: javascript}[Utilizzo di Java.](/docs/services/cloud-object-storage/libraries?topic=cloud-object-storage-java){:new_window}{: java}[Utilizzo di Go.](/docs/services/cloud-object-storage/libraries?topic=cloud-object-storage-go){:new_window}{: go}

## Esempio di codice
{: #sdk-gs-example}

Gli esempi di codice di seguito forniscono degli esempi introduttivi dell'esecuzione delle operazioni di base con {{site.data.keyword.cos_short}}. Per semplicità, l'esempio di codice può essere eseguito più volte in quanto utilizza gli UUID (Universally Unique Identifier) per i nomi di bucket/elemento per evitare potenziali conflitti.

Per completare l'esempio di codice, devi sostituire i seguenti valori:

|Valore|Descrizione|Esempio|
|---|---|---|
|`<endpoint>`|Endpoint regionale per la tua istanza COS|`s3.us-south.cloud-object-storage.appdomain.cloud`|
|`<api-key>`|Chiave API IAM con almeno le autorizzazioni `Writer`|`xxxd12V2QHXbjaM99G9tWyYDgF_0gYdlQ8aWALIQxXx4`|
|`<resource-instance-id>`|ID univoco per l'istanza del servizio|`crn:v1:bluemix:public:cloud-object-storage:global:a/xx999cd94a0dda86fd8eff3191349999:9999b05b-x999-4917-xxxx-9d5b326a1111::`|
|`<storage-class>`|Classe di archiviazione per un nuovo bucket|`us-south-standard`|
...
## Esecuzione dell'esempio di codice
{: #sdk-gs-run}

Per eseguire l'esempio di codice, copia i blocchi di codice in alto ed esegui:
```
...
## Output per l'esempio di codice
{: #sdk-gs-output}

L'output del codice di esempio dovrebbe somigliare a quanto segue:
