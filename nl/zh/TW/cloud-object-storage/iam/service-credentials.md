...
服務認證提供必要的資訊，以將應用程式連接至以 JSON 文件包裝的 {{site.data.keyword.cos_short}}。服務認證一律會與「服務 ID」相關聯，而且可以建立新的「服務 ID」與新的認證。 

使用下列步驟，來建立服務認證：
1. 登入 {{site.data.keyword.cloud_notm}} 主控台，並導覽至 {{site.data.keyword.cos_short}} 的實例。
2. 在側邊導覽中，按一下**服務認證**。 
3. 按一下**新建認證**，並提供必要資訊。如果您要產生 HMAC 認證，請按一下「包括 HMAC 認證」勾選框。
...
欄位名稱 | 值
--|--
`apikey`  |  為「服務 ID」建立新的 API 金鑰
`cos_hmac_keys`  |  「存取金鑰/密碼金鑰」配對，以與 S3 相容工具及程式庫搭配使用
`endpoints`  |  鏈結至可用端點的 JSON 表示法
`iam_apikey_description`  |  API 金鑰說明 - 一開始會自動產生，但可編輯
`iam_apikey_name`  |  API 金鑰名稱 - 一開始會自動產生，但可編輯
`iam_role_crn`  |  所指派角色的唯一 ID
`iam_serviceid_crn`  |  「服務 ID」的唯一 ID
`resource_instance_id`  |  認證將存取的 {{site.data.keyword.cos_short}} 實例的唯一 ID。這也稱為服務認證。
...
提供作為服務認證一部分的 `endpoints` URL，提供可在連接用戶端時使用的所有可能端點清單：
...
建立服務認證時，基礎「服務 ID」會獲授與整個 {{site.data.keyword.cos_short}} 實例上的角色。如果要使用認證來授與對儲存區子集的存取權，而不是整個實例的存取權，則需要編輯此原則。如需詳細資料，請參閱[儲存區許可權](/docs/services/cloud-object-storage/iam?topic=cloud-object-storage-iam-bucket-permissions)頁面。

## API 金鑰與 HMAC
{: #service-credentials-iam-hmac}

一般而言，「IAM API 金鑰」是 {{site.data.keyword.cos_full}} 偏好的鑑別方法。主要支援 HMAC，以取得從 IaaS {site.data.keyword.cos_short}} 及舊式 S3 應用程式移轉的應用程式舊版相容性。使用 COS SDK 開發應用程式時，原本也支援 IAM。會自動處理記號有效期限及重新整理，以簡化處理程序。


如需 IAM 的相關資訊，請造訪[開始使用 IAM](/docs/services/cloud-object-storage/iam?topic=cloud-object-storage-iam-overview)

如需 HMAC 的相關資訊，請造訪[使用 HMAC 認證](/docs/services/cloud-object-storage/hmac?topic=cloud-object-storage-hmac)
