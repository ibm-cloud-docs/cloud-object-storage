...
* [{{site.data.keyword.cloud}} 平台帳戶](https://cloud.ibm.com/login)
* [{{site.data.keyword.cos_full_notm}} 的實例](/docs/services/cloud-object-storage/basics?topic=cloud-object-storage-provision)
* [IAM API 金鑰](/docs/services/cloud-object-storage/iam?topic=cloud-object-storage-iam-overview)，且具有對 {{site.data.keyword.cos_short}} 的撰寫者存取權

## 取得 SDK
{: #sdk-gs-install}

下載及安裝 SDK 的特定指示，位於[使用 Python。](/docs/services/cloud-object-storage/libraries?topic=cloud-object-storage-python){:new_window}{: python}[使用 Node.js。](/docs/services/cloud-object-storage/libraries?topic=cloud-object-storage-node){:new_window}{: javascript}[使用 Java。](/docs/services/cloud-object-storage/libraries?topic=cloud-object-storage-java){:new_window}{: java}[使用 Go。](/docs/services/cloud-object-storage/libraries?topic=cloud-object-storage-go){:new_window}{: go}

## 程式碼範例
{: #sdk-gs-example}

下面的程式碼範例提供使用 {{site.data.keyword.cos_short}} 執行基本作業的入門範例。為了簡單起見，程式碼範例可以執行多次，因為它針對儲存區/項目名稱使用「通用唯一 ID (UUID)」，以避免可能的衝突。

若要完成程式碼範例，您需要取代下列值：

|值|說明|範例 |
|---|---|---|
|`<endpoint>`|COS 實例的地區端點|`s3.us-south.cloud-object-storage.appdomain.cloud`|
|`<api-key>`|具有至少 `Writer` 許可權的 IAM API 金鑰|`xxxd12V2QHXbjaM99G9tWyYDgF_0gYdlQ8aWALIQxXx4`|
|`<resource-instance-id>`|服務實例的唯一 ID|`crn:v1:bluemix:public:cloud-object-storage:global:a/xx999cd94a0dda86fd8eff3191349999:9999b05b-x999-4917-xxxx-9d5b326a1111::`|
|`<storage-class>`|新儲存區的儲存空間類別|`us-south-standard`|
