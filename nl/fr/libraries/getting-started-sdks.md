...
Ce guide de démarrage rapide contient un exemple de code qui permet d'illustrer les opérations suivantes :

* Création d'un nouveau compartiment
* Création de la liste des compartiments disponibles
* Création d'un nouveau fichier texte
* Création de la liste des fichiers disponibles
* Extraction du contenu de fichier texte
* Envoi par téléchargement d'un fichier binaire volumineux
...
* Un [compte de plateforme {{site.data.keyword.cloud}}](https://cloud.ibm.com/login) 
...
* Une [clé d'API IAM](/docs/services/cloud-object-storage/iam?topic=cloud-object-storage-iam-overview) et des droits d'accès Editeur à votre {{site.data.keyword.cos_short}}

## Obtention du SDK
{: #sdk-gs-install}

Des instructions spécifiques pour l'envoi par téléchargement et l'installation du SDK sont disponibles dans les rubriques [Utilisation de Python](/docs/services/cloud-object-storage/libraries?topic=cloud-object-storage-python){:new_window}, {: python}[Utilisation de Node.js](/docs/services/cloud-object-storage/libraries?topic=cloud-object-storage-node){:new_window}, {: javascript}[Utilisation de Java](/docs/services/cloud-object-storage/libraries?topic=cloud-object-storage-java){:new_window} et {: java}[Utilisation de Go](/docs/services/cloud-object-storage/libraries?topic=cloud-object-storage-go){:new_window}. {: go}

## Exemple de code
{: #sdk-gs-example}

Les exemples de code ci-après fournissent des exemples d'exécution des opérations de base avec {{site.data.keyword.cos_short}}. Par souci de simplicité, l'exemple de code peut être exécuté plusieurs fois car il utilise des identificateurs uniques universels (UUID) pour les noms de compartiment/d'élément afin de prévenir tout conflit. 

Pour exécuter l'exemple de code, vous devez remplacer les valeurs suivantes :

| Valeur  |Description|Exemple|
|---|---|---|
|`<endpoint>`|Noeud final régional pour votre instance COS|`s3.us-south.cloud-object-storage.appdomain.cloud`|
|`<api-key>`|Clé d'API IAM avec au minimum des droits d'accès `Writer`|`xxxd12V2QHXbjaM99G9tWyYDgF_0gYdlQ8aWALIQxXx4`|
|`<resource-instance-id>`|ID unique de l'instance de service|`crn:v1:bluemix:public:cloud-object-storage:global:a/xx999cd94a0dda86fd8eff3191349999:9999b05b-x999-4917-xxxx-9d5b326a1111::`|
|`<storage-class>`|Classe de stockage d'un nouveau compartiment|`us-south-standard`|
...
Pour exécuter l'exemple de code, copiez les blocs de code ci-dessus et exécutez les commandes suivantes :
```
...
Le résultat de l'exemple de code devrait se présenter comme suit :
