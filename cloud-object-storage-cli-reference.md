...
If you are using IAM authentication, then you then you must provide a CRN to use some of the commands. To set the CRN, you can type `ibmcloud cos config crn` and provide your CRN. You can find the CRN with `ibmcloud resource service-instance INSTANCE_NAME`.  Alternatively, you might open the web-based console, select **Service credentials** in the sidebar, and create a new set of credentials (or view an existing credential file that you already created).
