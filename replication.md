...
- Only new objects or new versions of the existing objects (created after the replication rule was added to the bucket) are copied to the destination bucket. Existing objects can be replicated [by copying them onto themselves](/docs/cloud-object-storage?topic=cloud-object-storage-replication-overview#replication-existing), creating a new version that is replicated.
