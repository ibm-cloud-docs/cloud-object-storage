...
To configure the AWS CLI, type `aws configure`. Provide your [HMAC credentials](/docs/cloud-object-storage/hmac?topic=cloud-object-storage-hmac) and a default region name. The "region name" used by AWS S3 corresponds to the code (`LocationConstraint`) that {{site.data.keyword.cos_short}} uses to define a storage class.
